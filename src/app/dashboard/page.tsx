...
export default async function DashboardPage() {
  ...
  // Changed from "awaitingReplyCount"
  const { count: inProgressTicketsCount } = await supabase
    .from("tickets")
    .select("*", { count: "exact", head: true })
    .eq("created_by", user.id)
    .eq("status", "In Progress"); // Changed from "Awaiting Reply"
  ...
  const { data: openTickets } = await supabase
    ...
    .eq("created_by", user.id)
    .in("status", ["Open", "In Progress"]) // Removed "Awaiting Reply"
    ...
    <main className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="md:col-span-1 lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          />
          <StatCard
            title="In Progress" // Changed from "Awaiting Your Reply"
            value={inProgressTicketsCount ?? 0} // Changed from awaitingReplyCount
            icon={<MailQuestion />}
          />
          <StatCard
